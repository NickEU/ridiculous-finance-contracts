...
import {IRidiPair} from "./interfaces/IRidiPair.sol";
import {IRidiRouter02} from "./interfaces/IRidiRouter02.sol";
 ...
 */
contract RidiZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped FTM (WFTM)
    IWETH public WFTM;

    // RidiRouter interface
    IRidiRouter02 public RidiRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from RidiSwap)
    ...
    // Address RidiRouter
    address private RidiRouterAddress;

    // Address Wrapped FTM (WFTM)
    address private WFTMAddress;
    ...
    // tokenToZap = 0x00 address if FTM
    event ZapIn(
    ...
    // token0ToZap = 0x00 address if FTM
    ...
    // tokenToReceive = 0x00 address if FTM
    event ZapOut(
        address indexed lpToken,
    ...
    /*
     * @notice Fallback for WFTM
     */
    receive() external payable {
        assert(msg.sender == WFTMAddress);
    }

    /*
     * @notice Constructor
     * @param _WFTMAddress: address of the WFTM contract
     * @param _RidiRouter: address of the RidiRouter
    ...
    constructor(
        ...
        WFTM = IWETH(_WFTMAddress);
        RidiRouterAddress = _RidiRouter;
        RidiRouter = IRidiRouter02(_RidiRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap FTM in a WFTM pool (e.g. WFTM/token)
     * @param _lpToken: LP token address (e.g. RIDI/FTM)
     * @param _tokenAmountOutMin: minimum token amount (e.g. RIDI) to receive in the intermediary swap (e.g. FTM --> RIDI)
     */
    function zapInFTM(address _lpToken, uint256 _tokenAmountOutMin) external payable nonReentrant {
        WFTM.deposit{value: msg.value}();

        // Call zap function
        uint256 lpTokenAmountTransferred = _zapIn(WFTMAddress, msg.value, _lpToken, _tokenAmountOutMin);
     ...
     * @param _lpToken: LP token address (e.g. RIDI/USDT)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. RIDI) in the intermediary swap (e.g. USDT --> RIDI)
     */
    function zapInToken(
        address _tokenToZap,
    ...
    /*
     * @notice Zap 1 token and FTM, rebalance them to 50-50, before adding them to LP
     * @param _token1ToZap: address of token1 to zap
     * @param _token1AmountIn: amount of token1 to zap
    ...
    /*
     * @notice Zap a LP token out to receive FTM
     * @param _lpToken: LP token address (e.g. RIDI/WFTM)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in FTM/WFTM) in the intermediary swap (e.g. RIDI --> FTM)
     */
    function zapOutFTM(
        address _lpToken,
        ...
        // Call zapOut
        uint256 tokenAmountToTransfer = _zapOut(_lpToken, WFTMAddress, _tokenAmountOutMin);

        // Unwrap FTM
        WFTM.withdraw(tokenAmountToTransfer);

        // Transfer FTM to the msg.sender
        (bool success, ) = msg.sender.call{value: tokenAmountToTransfer}(new bytes(0));
        require(success, "FTM: transfer fail");
     ...
     * @param _lpToken: LP token address (e.g. RIDI/USDT)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. RIDI or USDT)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. RIDI) in the intermediary swap (e.g. USDT --> RIDI)
     */
     ...
     * @dev Use WFTM for _tokenToZap (if FTM is the input)
    ...
    {
        address token0 = IRidiPair(_lpToken).token0();
        address token1 = IRidiPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IRidiPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = RidiRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = RidiRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
     ...
     * @dev Use WFTM for _token0ToZap (if FTM is the input)
    ...
    {
        require(
            _token0ToZap == IRidiPair(_lpToken).token0() || _token0ToZap == IRidiPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IRidiPair(_lpToken).token0() || _token1ToZap == IRidiPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IRidiPair(_lpToken).token0()) {
    ...
    /*
     * @notice View the details for single zap
     * @dev Use WFTM for _tokenToReceive (if FTM is the asset to be received)
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IRidiPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IRidiPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = RidiRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = RidiRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IRidiPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "RidiRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "RidiRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        RidiRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), RidiRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(RidiRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = RidiRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = RidiRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = RidiRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = RidiRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = RidiRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
